...
<body>
    ...
    <div class="container">
        <div class="Tieu_de" style="margin-top: 10px;">
            <div>
                Tìm kiếm: <?php echo $Search ?>
            </div>
        </div>
        <div class="row">
            <?php
            $data_page = [];
            $sql_page = "SELECT COUNT(sanpham.ID) as SoLuong FROM sanpham INNER JOIN xuatxu ON sanpham.XuatXu = xuatxu.ID WHERE TenSanPham LIKE '%$Search%' OR GiaBan = '$Search' OR xuatxu.XuatXu LIKE '%$Search%'";
            ...
            if($pages != 0){
                ...
                $data = [];
                $sql = "SELECT sanpham.* FROM sanpham INNER JOIN xuatxu ON sanpham.XuatXu = xuatxu.ID WHERE TenSanPham LIKE '%$Search%' OR GiaBan = '$Search' OR xuatxu.XuatXu LIKE '%$Search%' ORDER BY NgayNhap DESC LIMIT $index, 12";       
            ...
            <div class="Phan_Trang">
                <ul class="pagination pagination-lg">
                    <?php
                        for($i = 0; $i < $pages; $i++){
                            echo '<li class="page-item"><a class="page-link" href="?Search='.$Search.'&page='.($i + 1).'">'.($i + 1).'</a></li>';
                        }
                    ?>
                </ul>
            </div>
